...
int angleID = 0;
float angleX[10] = { };
float angleY[10] = { };///float angle=0, da=-1; ///加入旋轉
void display()
{
    ...
    glPushMatrix();
        ...
        glPushMatrix(); /// Step 05
            ...
            glRotatef(angleY[1], 0, 1, 0); ///Step02(2) 旋轉
            glTranslatef(-0.025, -0.05, 0); ///(1)
            drawUpper(); ///Step 03
            glPushMatrix(); ///加入旋轉
                glTranslatef(0, -0.1, 0); ///Step 04 (3) 把旋轉中的手,掛到肩上
                glRotatef(angleX[2], 1, 0, 0);
                glRotatef(angleY[2], 0, 1, 0); ///Step02(2) 旋轉
                glTranslatef(-0.02, -0.2, 0); ///Step 01(1) 把旋轉中心,放到正中心 (有個球在正中心)
                drawLower(); /// Step 00
            glPopMatrix(); ///加入旋轉
        glPopMatrix(); /// Step 05
    glPopMatrix();
